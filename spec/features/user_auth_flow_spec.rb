...
RSpec.describe 'User Auth Flow', type: :request do
  def headers(user_id = nil, token = nil)
    ...
    if user_id && token
      ...
      { 'HTTP_AUTHORIZATION' =>
            "Sandra-Token api_key=#{api_key_str}, access_token=#{token_str}" }
    else
      { 'HTTP_AUTHORIZATION' =>
            "Sandra-Token api_key=#{api_key.id}:#{api_key.key}" }
    end
  end

  let(:api_key) { ApiKey.first || ApiKey.create }
  let(:email) { 'daniel@sandra.app' }
  let(:password) { 'password' }
  let(:params) { { email: email, password: password, given_name: 'Daniel' } }
  ...
  it 'authenticate a new user' do
    ...
    patch "/api/users/#{id}", params: { data: { given_name: 'John' } }, headers: headers
    expect(response.status).to eq 401

    # Step 3 - Login
    post '/api/access_tokens', params: { data: { email: email, password: 'password' } }, headers: headers
    expect(response.status).to eq 201
    ...
    patch "/api/users/#{id}", params: { data: { given_name: 'Vasile' } }, headers: headers(user_id, token)
