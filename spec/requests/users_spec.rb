...
RSpec.describe 'Users', type: :request do
  ...
  describe 'GET /api/users' do
    ...
    xdescribe 'embed picking' do
      context 'with valid "embed" parameter' do
        before { get '/api/users?embed=author' }

        it 'gets the users with their authors embedded' do
          json_body['data'].each do |user|
            expect(user['author'].keys).to eq(['id', 'given_name', 'family_name', 'created_at', 'updated_at'])
          end
        end
      end

      context 'with invalid "embed" relation "fake"' do
        before { get '/api/users?embed=fake,author' }
