  ...
  describe 'GET /api/user_confirmations/:confirmation_token' do
    context 'with existing token' do
      context 'with confirmation redirect url' do
        subject { get "/api/user_confirmations/#{daniel.confirmation_token}" }
        let(:daniel) { create(:user, :confirmation_redirect_url) }
      ...
      context 'without confirmation redirect url' do
        let(:daniel) { create(:user, :confirmation_no_redirect_url) }
        before { get "/api/user_confirmations/#{daniel.confirmation_token}" }
